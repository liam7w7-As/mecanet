...
describe('Sequelize Models & Database Integration (unithor_test)', () => {
  ...
  it('crea WorkOrder + WorkOrderItem y verifica eliminación en cascada', async () => {
    ...
    const photo = await WorkOrderInspectionPhoto.create({
      inspectionId: inspection.id,
      slot: 'frontal',
      storageKey: 'test/photo.png',
      mimeType: 'image/png',
      sizeBytes: 8,
      sha256: 'a'.repeat(64),
    });
    ...
    const delivery = await WorkOrderDelivery.create({
      workOrderId: workOrder.id,
      kilometrajeSalida: 120500,
      receptorNombre: 'Juan Soto',
      receptorRut: null,
    ...
    const event = await WorkOrderEvent.create({
      ...
      actorUserId: null,
      tipo: 'creacion',
      descripcion: 'Orden creada en prueba',
      metadata: { source: 'models.test' },
    });
    ...
    const remainingPhotos = await WorkOrderInspectionPhoto.findAll({
      where: { inspectionId: inspection.id },
    });
    expect(remainingPhotos.length).toBe(0);
    expect(await WorkOrderDelivery.count({ where: { workOrderId: workOrder.id } })).toBe(0);
    expect(await WorkOrderEvent.count({ where: { workOrderId: workOrder.id } })).toBe(0);
  });
