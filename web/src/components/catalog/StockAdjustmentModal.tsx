...
export const StockAdjustmentModal = ({ item, onClose }: StockAdjustmentModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4 py-6">
      ...
      <section className="relative w-full max-w-lg rounded-lg bg-white shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="stock-adjustment-title">
        <header className="flex items-start justify-between border-b border-slate-200 px-5 py-4 sm:px-6">
          ...
          <button type="button" className="flex h-9 w-9 items-center justify-center rounded-lg text-slate-500 hover:bg-slate-100" onClick={onClose} aria-label="Cerrar" title="Cerrar"><X className="h-5 w-5" aria-hidden="true" /></button>
        </header>

        <form className="space-y-5 p-5 sm:p-6" noValidate onSubmit={handleSubmit}>
          <div className="grid grid-cols-2 gap-3">
            <button type="button" className={`flex min-h-11 items-center justify-center gap-2 rounded-lg border text-sm font-semibold ${movementType === 'entrada' ? 'border-emerald-600 bg-emerald-50 text-emerald-800' : 'border-slate-300 text-slate-600 hover:bg-slate-50'}`} onClick={() => { setMovementType('entrada'); setFieldErrors({}); }} aria-pressed={movementType === 'entrada'}>
              <ArrowDownToLine className="h-4 w-4" aria-hidden="true" /> Entrada / Ingreso
            </button>
            <button type="button" className={`flex min-h-11 items-center justify-center gap-2 rounded-lg border text-sm font-semibold ${movementType === 'salida' ? 'border-red-500 bg-red-50 text-red-700' : 'border-slate-300 text-slate-600 hover:bg-slate-50'}`} onClick={() => { setMovementType('salida'); setFieldErrors({}); }} aria-pressed={movementType === 'salida'}>
              <ArrowUpFromLine className="h-4 w-4" aria-hidden="true" /> Salida / Egreso
            </button>
          </div>
          ...
          <footer className="flex justify-end gap-2 border-t border-slate-100 pt-5">
            ...
            <button type="submit" className="inline-flex h-10 items-center gap-2 rounded-lg bg-brand-blue px-5 text-sm font-semibold text-white hover:bg-brand-dark disabled:opacity-60" disabled={adjustmentMutation.isPending}>
