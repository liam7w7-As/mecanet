    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto px-4 py-4 sm:items-center sm:py-6">
      ...
      >
        <header className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-200 bg-white px-5 py-4 sm:px-6">
          ...
          <button
            type="button"
            className="group flex h-9 w-9 items-center justify-center rounded-lg text-slate-500 hover:bg-slate-100"
            onClick={onClose}
            aria-label="Cerrar"
            title="Cerrar"
          >
        ...
        {clientQuery.isError && (
          <div
            className="m-6 flex items-start gap-2 rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-700"
            role="alert"
          >
        ...
        {clientQuery.data && (
          <div className="space-y-6 p-5 sm:p-6">
            <div className="flex flex-wrap items-center gap-2">
              <span
                className={`rounded px-2 py-1 text-xs font-semibold ${clientQuery.data.tipo === 'empresa' ? 'bg-amber-100 text-amber-800' : 'bg-blue-100 text-blue-800'}`}
              >
                {clientQuery.data.tipo === 'empresa' ? 'Empresa' : 'Persona natural'}
              </span>
              <span className="text-sm font-medium text-slate-600">
                {clientQuery.data.rut ?? 'Sin identificación'}
              </span>
            </div>
            ...
            <dl className="divide-y divide-slate-100 border-y border-slate-100 text-sm">
              <div className="grid gap-1 py-3 sm:grid-cols-[9rem_1fr]">
                ...
                <dd className="font-medium text-slate-800">
                  {clientQuery.data.telefono ?? 'No registrado'}
                </dd>
              </div>
              <div className="grid gap-1 py-3 sm:grid-cols-[9rem_1fr]">
                <dt className="flex items-center gap-2 text-slate-500">
                ...
                <dd className="break-all font-medium text-slate-800">
                  {clientQuery.data.email ?? 'No registrado'}
                </dd>
              </div>
              <div className="grid gap-1 py-3 sm:grid-cols-[9rem_1fr]">
                <dt className="flex items-center gap-2 text-slate-500">
                ...
                <dd className="font-medium text-slate-800">
                  {[clientQuery.data.direccion, clientQuery.data.comuna, clientQuery.data.region]
                    .filter(Boolean)
                    .join(', ') || 'No registrada'}
                </dd>
              </div>
            ...
            <section aria-labelledby="client-vehicles-title">
              <div className="mb-3 flex items-center justify-between">
                <h3
                  id="client-vehicles-title"
                  className="flex items-center gap-2 font-semibold text-brand-blue"
                >
                  <Car className="h-5 w-5" aria-hidden="true" /> Vehículos asociados
                </h3>
                <span className="text-sm text-slate-500">
                  {clientQuery.data.vehicles?.length ?? 0}
                </span>
              </div>
              {clientQuery.data.vehicles?.length ? (
                ...
                <p className="border-y border-slate-100 py-6 text-center text-sm text-slate-500">
            ...
            {canDelete && isConfirmingDelete && (
              <div className="rounded-lg border border-red-100 bg-red-50 p-3 text-sm text-red-700">
                ...
                <p className="mt-1">
                  El registro se ocultará de los listados y conservará su historial.
                </p>
                <div className="mt-3 flex gap-2">
                  <button
                    type="button"
                    className="h-9 rounded-lg border border-red-200 bg-white px-3 font-semibold text-red-700"
                  ...
                  <button
                    type="button"
                    className="inline-flex h-9 items-center gap-2 rounded-lg bg-red-700 px-3 font-semibold text-white hover:bg-red-800 disabled:opacity-60"
