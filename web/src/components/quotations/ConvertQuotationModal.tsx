...
export const ConvertQuotationModal = ({ quotationId, codigo, notas, onClose, onConverted }: ConvertQuotationModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <section className="relative w-full max-w-lg rounded-lg bg-white p-6 shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="convert-title">
        ...
        <form onSubmit={submit} className="mt-5 space-y-4">
          ...
          <div className="flex justify-end gap-2 pt-2"><button type="button" className="h-10 rounded-lg border border-slate-300 px-4 text-sm font-semibold text-slate-700" onClick={onClose}>Cancelar</button><button type="submit" className="inline-flex h-10 items-center gap-2 rounded-lg bg-brand-yellow px-4 text-sm font-bold text-brand-dark disabled:opacity-60" disabled={conversionMutation.isPending}>{conversionMutation.isPending ? <LoaderCircle className="h-4 w-4 animate-spin" aria-hidden="true" /> : <ArrowRight className="h-4 w-4" aria-hidden="true" />}Crear Orden de Trabajo</button></div>
        </form>
      </section>
    </div>
  );
