...
const AvailabilityBadge = ({ item }: { item: CatalogItem }) => {
  ...
  if (item.stock === 0) {
    return <span className="inline-flex rounded bg-red-100 px-2 py-1 text-xs font-semibold text-red-700">Sin stock</span>;
  }
  if (item.stock <= 5) {
    return <span className="inline-flex rounded bg-amber-100 px-2 py-1 text-xs font-semibold text-amber-800">Stock crítico: {item.stock}</span>;
  }
  return <span className="inline-flex rounded bg-emerald-100 px-2 py-1 text-xs font-semibold text-emerald-800">{item.stock} unidades</span>;
};
  ...
  return (
    <div className="space-y-5">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        {canCreate && <button type="button" className="inline-flex h-10 items-center justify-center gap-2 rounded-lg bg-brand-yellow px-4 text-sm font-bold text-brand-dark hover:bg-yellow-400" onClick={() => setFormItem(null)}><Plus className="h-4 w-4" aria-hidden="true" />Nuevo item</button>}
      </header>

      <section className="overflow-hidden border border-slate-200 bg-white" aria-label="Listado de catálogo">
        <div className="space-y-4 border-b border-slate-200 p-4">
          <label className="relative block max-w-xl">
            ...
            <input className="h-10 w-full rounded-lg border border-slate-300 pl-9 pr-3 text-sm outline-none focus:border-brand-blue focus:ring-2 focus:ring-brand-blue/15" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar por código o nombre" aria-label="Buscar en catálogo" />
          </label>
          <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
            <div className="flex gap-2 overflow-x-auto pb-1" role="tablist" aria-label="Filtrar catálogo por tipo">
              {tabs.map((catalogTab) => <button key={catalogTab.value} type="button" role="tab" aria-selected={tab === catalogTab.value} className={`h-9 shrink-0 rounded-lg px-3 text-sm font-semibold ${tab === catalogTab.value ? 'bg-brand-blue text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`} onClick={() => selectTab(catalogTab.value)}>{catalogTab.label}</button>)}
            </div>
            {tab === 'parte' && (
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[920px] text-left text-sm">
            ...
            <tbody>
              {catalogQuery.isPending ? <CatalogSkeleton /> : catalogQuery.data?.items.map((item) => (
                <tr key={item.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/70">
                  ...
                  <td className="px-4 py-3"><div className="flex justify-end gap-1">
                    {canEdit && <button type="button" className="flex h-8 w-8 items-center justify-center rounded-lg text-slate-500 hover:bg-amber-50 hover:text-amber-700" onClick={() => setFormItem(item)} aria-label={`Editar ${item.nombre}`} title="Editar"><Pencil className="h-4 w-4" aria-hidden="true" /></button>}
                    {canEdit && <button type="button" className="flex h-8 w-8 items-center justify-center rounded-lg text-slate-500 hover:bg-blue-50 hover:text-brand-blue disabled:cursor-not-allowed disabled:opacity-30" onClick={() => setStockItem(item)} disabled={item.tipo !== 'parte'} aria-label={`Ajustar stock de ${item.nombre}`} title={item.tipo === 'parte' ? 'Ajustar stock' : 'Solo disponible para repuestos'}><Boxes className="h-4 w-4" aria-hidden="true" /></button>}
                    {canDelete && <button type="button" className="flex h-8 w-8 items-center justify-center rounded-lg text-slate-500 hover:bg-red-50 hover:text-red-700" onClick={() => { deleteMutation.reset(); setDeleteItem(item); }} aria-label={`Eliminar ${item.nombre}`} title="Eliminar"><Trash2 className="h-4 w-4" aria-hidden="true" /></button>}
                  </div></td>
                </tr>
              ))}
            </tbody>
