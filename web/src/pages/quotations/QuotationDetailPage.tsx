...
export const QuotationDetailPage = () => {
  ...
  const pendingTransferAmount = summary?.payments
    .filter((payment) => payment.estado === 'por_verificar')
    .reduce((sum, payment) => sum + payment.monto, 0) ?? 0;
  const availableBalance = Math.max(0, balance - pendingTransferAmount);
  ...
  return (
    <div className="min-w-0 space-y-5">
      ...
      <section className="overflow-hidden rounded-lg border border-slate-200 bg-white" aria-labelledby="quotation-items-title"><div className="border-b border-slate-200 px-5 py-4"><h2 id="quotation-items-title" className="font-bold text-brand-blue">Conceptos cotizados</h2></div><div className="max-w-full overflow-x-auto overscroll-x-contain"><table className="w-full min-w-[920px] text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="px-4 py-3 font-semibold">Descripción</th><th className="px-4 py-3 font-semibold">Avance taller</th><th className="px-4 py-3 font-semibold">Nota operativa</th><th className="px-4 py-3 text-right font-semibold">Cantidad</th><th className="px-4 py-3 text-right font-semibold">Precio unitario</th><th className="px-4 py-3 text-right font-semibold">Subtotal</th></tr></thead><tbody>{quotation.items?.map((item) => <tr key={item.id} className="border-t border-slate-100"><td className="px-4 py-3 font-medium text-slate-800">{item.descripcion}</td><td className="px-4 py-3"><span className="rounded-md bg-slate-100 px-2 py-1 text-xs font-semibold text-slate-700">{ITEM_OPERATIONAL_STATUS_LABELS[item.estadoOperativo] ?? item.estadoOperativo}</span></td><td className="max-w-64 px-4 py-3 text-slate-600">{item.notasOperativas ?? '-'}</td><td className="px-4 py-3 text-right text-slate-600">{item.cantidad}</td><td className="px-4 py-3 text-right text-slate-600">{formatClp(item.precioUnitario)}</td><td className="px-4 py-3 text-right font-semibold text-brand-blue">{formatClp(item.subtotal)}</td></tr>)}</tbody></table></div>{quotation.items?.length === 0 && <p className="px-5 py-8 text-center text-sm text-slate-500">Sin conceptos cotizados.</p>}</section>

      <section className="overflow-hidden rounded-lg border border-slate-200 bg-white" aria-labelledby="payments-title">
        <div className="flex flex-col gap-3 border-b border-slate-200 px-5 py-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 id="payments-title" className="font-bold text-brand-blue">Historial de pagos y abonos</h2><p className="mt-1 text-sm text-slate-500">Todos los movimientos registrados para esta cotización.</p></div>{canCreatePayment && availableBalance > 0 && <button type="button" className="inline-flex h-10 items-center justify-center gap-2 rounded-lg bg-brand-blue px-4 text-sm font-semibold text-white hover:bg-brand-dark" onClick={() => setShowPaymentModal(true)}><Plus className="h-4 w-4" aria-hidden="true" /> Registrar Abono</button>}</div>
        <div className="max-w-full overflow-x-auto overscroll-x-contain"><table className="w-full min-w-[780px] text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="px-4 py-3 font-semibold">Fecha</th><th className="px-4 py-3 font-semibold">Método</th><th className="px-4 py-3 font-semibold">Estado</th><th className="px-4 py-3 font-semibold">Usuario receptor</th><th className="px-4 py-3 text-right font-semibold">Monto</th><th className="px-4 py-3 text-right font-semibold">Acción</th></tr></thead><tbody>{summary?.payments.map((payment) => {
          const paymentStatus = payment.estado ?? 'confirmado';
          return <tr key={payment.id} className="border-t border-slate-100"><td className="whitespace-nowrap px-4 py-3 text-slate-600">{formatDateTime(payment.fecha)}</td><td className="px-4 py-3 font-medium text-slate-800">{getPaymentMethodLabel(payment.metodo)}</td><td className="px-4 py-3"><span className={`rounded px-2 py-1 text-xs font-semibold ${PAYMENT_STATUS_STYLES[paymentStatus]}`}>{PAYMENT_STATUS_LABELS[paymentStatus]}</span></td><td className="px-4 py-3 text-slate-600">{payment.creator?.nombre ?? 'Sin registro'}</td><td className={`px-4 py-3 text-right font-semibold ${paymentStatus === 'confirmado' ? 'text-emerald-700' : 'text-slate-600'}`}>{formatClp(payment.monto)}</td><td className="px-4 py-3"><div className="flex justify-end">{canDeletePayment && <button type="button" className="flex h-9 w-9 items-center justify-center rounded-lg text-slate-400 hover:bg-red-50 hover:text-red-700" onClick={() => setPaymentToDelete(payment)} aria-label={`Anular pago de ${formatClp(payment.monto)}`} title="Anular pago"><Trash2 className="h-4 w-4" aria-hidden="true" /></button>}</div></td></tr>;
        })}</tbody></table></div>
        {paymentsQuery.isPending && <div className="flex min-h-28 items-center justify-center"><LoaderCircle className="h-5 w-5 animate-spin text-brand-blue" aria-label="Cargando pagos" /></div>}
        {!paymentsQuery.isPending && summary?.payments.length === 0 && <div className="flex min-h-36 flex-col items-center justify-center px-4 text-center"><Banknote className="h-8 w-8 text-slate-300" aria-hidden="true" /><p className="mt-2 text-sm font-semibold text-slate-600">Aún no hay abonos registrados</p></div>}
      </section>

      {showPaymentModal && summary && <PaymentFormModal quotationId={quotation.id} codigo={quotation.codigo} saldoPendiente={availableBalance} onClose={() => setShowPaymentModal(false)} />}
